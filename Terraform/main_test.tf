...
resource "openstack_compute_keypair_v2" "ssh_keypair8" {
  name = "ssh_keypair8"
}
...
resource "openstack_networking_secgroup_v2" "ssh_secgroup8" {
  name        = "ssh_secgroup8"
...
resource "openstack_networking_secgroup_rule_v2" "ssh" {
  ...
  security_group_id = openstack_networking_secgroup_v2.ssh_secgroup8.id
}
...
resource "openstack_compute_instance_v2" "vm" {
  ...
  key_pair        = openstack_compute_keypair_v2.ssh_keypair8.name
  security_groups = [openstack_networking_secgroup_v2.ssh_secgroup8.name]
...
resource "null_resource" "run_user_data" {
  depends_on = [
    openstack_compute_instance_v2.vm,
    ovh_cloud_project_database_ip_restriction.iprestriction
  ]
  ...
  provisioner "remote-exec" {
    connection {
      ...
      private_key = openstack_compute_keypair_v2.ssh_keypair8.private_key  # Path to your private key
    }
...
output "ssh_private_key" {
  value     = openstack_compute_keypair_v2.ssh_keypair8.private_key
  sensitive = true
}
